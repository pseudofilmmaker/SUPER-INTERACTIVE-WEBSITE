...
**코드 변경**:
- `src/pages/home.html`: `#ignite-gate`(원+링+CTA 텍스트), `#videos-connected-text`(단어별 `.vc-word` span 7개, 마침표 없음), `#videos-catchfire-text`(단어별 `.cf-word` span 7개, 마지막 단어에 `.cf-word--ember` 클래스) 오버레이 마크업 추가 (`#videos-bg-video-layer` 바로 뒤, `<main>` 앞)
